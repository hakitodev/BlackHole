const TAGS = [
    ...
    { tag: "{guild.id}", hint: "То же, что ID сервера." },
    { tag: "{count}", hint: "Сколько людей на сервере. Для массового удаления — сколько сообщений." },
    { tag: "{level}", hint: "Уровень игрока или новый уровень в ивенте." },
    { tag: "{xp}", hint: "Опыт игрока." },
    { tag: "{money}", hint: "Сколько монет упало за уровень." },
    { tag: "{balance}", hint: "Наличные в кошельке. Кастом-команды." },
    { tag: "{business.count}", hint: "Сколько бизнесов у игрока." },
...
const CUSTOM_TAGS = [
    "{user}", "{user.name}", "{user.id}", "{user.tag}",
    "{server}", "{server.id}", "{guild.id}", "{count}",
    "{balance}", "{level}", "{xp}", "{business.count}"
];

function tagsFor(kind, eventType) {
    if (kind === "custom") {
        const names = new Set(CUSTOM_TAGS);
        return TAGS.filter(item => names.has(item.tag));
    }
    if (eventType) {
        const names = new Set(String(eventType.hint || "").match(/\{[^}]+\}/g) || []);
        names.add("{user}");
        names.add("{user.name}");
        names.add("{user.id}");
        names.add("{server}");
        if (names.has("{server.id}")) {
            names.add("{guild.id}");
        }
        return TAGS.filter(item => names.has(item.tag));
    }
...
function fill(template, vars = {}) {
    ...
    return String(template ?? "")
        ...
        .replaceAll("{guild.id}", guild?.id ? String(guild.id) : "")
        ...
        .replaceAll("{business.count}", String(vars.businessCount ?? vars["business.count"] ?? ""))
