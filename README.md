...
Логин через Discord. Справа выпадающая панель: сервер, команды, кастомные слэш, ивенты. Высшие модеры — юзеры и всемирный шоп.
...
Экономика по умолчанию всемирная (старые балансы на месте). На сервере можно включить гильдийный кошелёк, локальные работы/бизнесы/магазин и штрафы за простой. Пассив бизнесов считается по `last_revenue_collect`, без таймеров. `/flip` режет ставку, `/rob` режет куш.

Сайт: тумблеры экономики, редактор локальных работ и бизнесов, цветные роли в магазине. Владелец бота может поставить сервер на паузу.
